...
import ida_kernwin
import sip
from PyQt5.QtGui import QPixmap  # noqa: I202
...
class Client(ClientSocket):
    ...
    def recv_packet(self, packet):
        self._plugin.logger.debug("Receive packet a ")
        if isinstance(packet, Command):
            # Call the corresponding handler
            self._plugin.logger.debug(packet)
    ...
    def _check_socket(self):
        self._plugin.logger.debug("Client check_socket")
    ...
    def _handle_update_notepad(self, query):
        """Update notepad in db"""
        text = query.text
