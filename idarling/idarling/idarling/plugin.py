...
from .interface.subdialogs.AuthentificationDialog import AuthentificationDialog
...
class Plugin(ida_idaapi.plugin_t):
    ...
    def __init__(self):
        ...
        self._network = Network(self, self._interface)
        self._token = None
        # fix for notepad
        self.nodepad_content = None
    ...
    def init(self):
        ...
        for server in self._config["servers"]:
            if "auto_connect" in server and server["auto_connect"]:
                ...
                dialog = AuthentificationDialog(self, server)
                if not dialog.exec_():
