...
from .subdialogs.AuthentificationDialog import AuthentificationDialog
...
class StatusWidget(QWidget):
    ...
    def refresh(self):
        ...
        elif bool(disc_servers):
            color, text, icon = "orange", "Available", "connecting.png"
        else:
        ...
        def create_servers_group(servers):
            ...
            def server_action_triggered(server_action):
                ...
                if not was_connected:
                    self._plugin.logger.debug("Servers settings : %s " % server)
                    self._plugin.network.connect(server)
                    dialog = AuthentificationDialog(self._plugin, server)
                    if not dialog.exec_():
                        self._plugin.network.disconnect()
                    if self._plugin.core.database is not None:
