...
from idarling.core.events import Event
...
class MyIDBHooks(ida_idp.IDB_Hooks):
    ...
    def enum_member_created(self, id, cid):
        ...
        print("Detected a new enum member created: 0x%x (%s), 0x%x (%s) = 0x%x, 0x%x" % (
    ...
    def struc_cmt_changed(self, id, repeatable_cmt):
        ...
        print(
            "Detected a struct comment changed for 0x%x (%s and %s): %s, %s" % (id, sname, smname, cmt, repeatable_cmt))
        return 0
    ...
    def segm_start_changed(self, s, oldstart):
        print("Detected a segment changed: %s start from 0x%x to 0x%x" % (
